...
export default function MoviePageSwiper(props) {
  const [movies] = useState([
    ...
    },
    {
      ...
      description:
        "Man of Steel is a 2013 superhero film based on the DC Comics character Superman. Directed by Zack Snyder and written by David S. Goyer, who developed the story with producer Christopher Nolan, it is the first film in the DC Extended Universe, and a reboot of the Superman film series, depicting the character's origin story. Henry Cavill stars as Superman",
      rating: 7,
      date: 2010,
      genre: "super hero",
    },
    ...
    },
    {
      ...
      description:
        "In 1989, Kevin Flynn, the CEO of ENCOM, disappears. Twenty years later, his son Sam, now ENCOM's primary shareholder, pranks the corporation by releasing the company's signature operating system online for free. ENCOM executive Alan Bradley, Kevin's old friend, approves of this, believing it aligns with Flynn's ideals of free software; nonetheless, Sam is arrested for trespassing. ",
      rating: 5,
      date: 2008,
      genre: "sci-fi",
    },
  ...
  return (
    <Swiper
      ...
      onSlideChange={(swiper) => {
        const activeMovie = movies[swiper.activeIndex];
        if (!activeMovie) return; // safety check for edge cases
        setMoviePoster(activeMovie["poster"]);
        setMovieTitle(activeMovie["title"]);
        setMovieTrailer(activeMovie["trailer"]);
        setMovieDate(activeMovie["date"]);
        setMovieGenre(activeMovie["genre"]);
        setMovieRating(activeMovie["rating"]);
        setMovieDescription(activeMovie["description"]);
      }}
      ...
      style={{
        "--swiper-pagination-color": "#5d09e5",
        "--swiper-pagination-bullet-inactive-color": "#4A4A4Ab3",
